--//====================================================
--// PAS - Precision Assist System V1.1 Pro Base
...
function PAS:UpdateAim()
	...
	local targetPosition = getPredictedPosition(target, target.AimPart)
...
local function getPredictedPosition(target, aimPart)
	local position = aimPart.Position

	if not PAS.Config.PredictionEnabled then
		return position
	end

	local root = target.Root or aimPart
	...
	local leadTime = (distance / 1000) * PAS.Config.PredictionStrength
	leadTime = math.clamp(leadTime, 0, PAS.Config.PredictionMaxLead / 10)
...
function PAS:CreateUI()
	...
	local title = create("TextLabel", {
		...
		BackgroundTransparency = 1,
		Text = "PAS V1.1 Pro  |  Precision Assist System",
		TextColor3 = Theme.Text,
		TextSize = 17,
		Font = Enum.Font.GothamBold,
	...
	local content = create("ScrollingFrame", {
		...
		CanvasSize = UDim2.fromOffset(0, 840),
		ZIndex = 31,
	}, main)
	...
	makeLabel("Prediction Engine")
	makeToggle("Predicción", "PredictionEnabled")
	makeNumberControl("Fuerza predicción", "PredictionStrength", 0.02, 0, 0.8)
	makeNumberControl("Límite predicción", "PredictionMaxLead", 1, 1, 12)
...
function PAS:RefreshDebug(force)
	...
	self.UI.DebugLabel.Text =
		...
		"\n Predicción: " .. tostring(self.Config.PredictionEnabled) ..
...
function PAS:Init()
	...
	print("[PAS] Precision Assist System V1.1 Pro Base cargado correctamente.")
end
